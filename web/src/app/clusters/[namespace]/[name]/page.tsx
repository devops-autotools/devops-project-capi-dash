...
import {
  ...
  Activity, Code, Info, Server, Cpu, AlertTriangle, XCircle,
  Terminal, Copy, Check, ExternalLink,
} from "lucide-react"

interface Condition {
  type: string; status: string; reason?: string
  message?: string; lastTransitionTime: string
}
interface ClusterDetail {
  name: string; namespace: string; phase: string; status: string
  infrastructure: string; controlPlaneReady: boolean
  infrastructureReady: boolean; createdAt: string
  conditions: Condition[]; raw: any
}
interface Machine {
...
interface NodeShellTool {
  available: boolean; installHint: string; usage: string; docs: string
}
...
// ── SSH Modal ──────────────────────────────────────────────────────────────
function SSHModal({ machine, tool, onClose }: {
  machine: Machine; tool: NodeShellTool | null; onClose: () => void
}) {
  const [copied, setCopied] = useState(false)
  const cmd = `kubectl node-shell ${machine.nodeName || machine.name}`
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-slate-900 p-2 rounded-lg"><Terminal size={18} className="text-emerald-400"/></div>
            <div>
              <h2 className="font-bold text-slate-900">Node Shell</h2>
              <p className="text-xs text-slate-500">{machine.nodeName || machine.name}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400"><XCircle size={18}/></button>
        </div>

        <div className="p-6 space-y-5">
          {/* Tool status */}
          <div className={`flex items-center gap-3 p-3 rounded-lg border ${
            tool?.available ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}>
            {tool?.available
              ? <CheckCircle2 size={16} className="text-emerald-600 flex-shrink-0"/>
              : <AlertTriangle size={16} className="text-amber-600 flex-shrink-0"/>}
            <div>
              <p className={`text-sm font-semibold ${tool?.available ? "text-emerald-700" : "text-amber-700"}`}>
                kubectl-node-shell {tool?.available ? "detected on management cluster" : "not found on management cluster"}
              </p>
              {!tool?.available && (
                <p className="text-xs text-amber-600 mt-0.5">Install: <code className="bg-amber-100 px-1 rounded">{tool?.installHint}</code></p>
              )}
            </div>
          </div>
          ...
          {/* Command */}
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase mb-2">Command</p>
            <div className="bg-slate-900 rounded-xl p-4 flex items-center justify-between gap-3">
              <code className="text-emerald-400 font-mono text-sm break-all">{cmd}</code>
              <button onClick={copy}
                className="flex-shrink-0 p-2 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white">
          ...
          <div className="bg-blue-50 rounded-lg p-4 text-sm text-blue-700 space-y-1">
            <p className="font-semibold flex items-center gap-1.5"><Info size={13}/> Cách dùng</p>
            <p className="text-xs text-blue-600">Chạy lệnh trên terminal đã có quyền truy cập Management Cluster:</p>
            <ol className="text-xs text-blue-600 list-decimal list-inside space-y-0.5">
              <li>Copy lệnh ở trên</li>
              <li>Mở terminal trên máy có <code className="bg-blue-100 px-1 rounded">KUBECONFIG</code> Management Cluster</li>
              <li>Paste và chạy lệnh → sẽ mở shell trực tiếp vào node</li>
            </ol>
          </div>

          {/* Links */}
          <a href="https://github.com/kvaps/kubectl-node-shell" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 text-xs text-slate-500 hover:text-blue-600 transition-colors">
            <ExternalLink size={12}/> kubectl-node-shell documentation
          </a>
        </div>
      </div>
    </div>
...
export default function ClusterDetailPage({ params }: { params: Promise<{ namespace: string; name: string }> }) {
  ...
  return (
    <div className="space-y-6">
      {/* SSH Modal */}
      {sshMachine && (
        <SSHModal machine={sshMachine} tool={nodeTool} onClose={() => setSshMachine(null)} />
      )}
      ...
      <div className="min-h-[400px]">
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: "Control Plane", ready: cluster.controlPlaneReady },
                    { label: "Infrastructure", ready: cluster.infrastructureReady },
                  ].map(s => (
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-3">
                ...
                <div className="flex justify-between border-b pb-2">
                  ...
                  <span className="font-medium font-mono text-xs">{cluster.raw?.spec?.controlPlaneEndpoint?.host || "N/A"}</span>
                </div>
              </div>
            </div>
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        {activeTab === 'machines' && (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {/* kubectl-node-shell availability hint */}
            {nodeTool && !nodeTool.available && (
              <div className="px-6 py-3 bg-amber-50 border-b border-amber-100 flex items-center gap-2 text-xs text-amber-700">
                <AlertTriangle size={13}/>
                <span><strong>kubectl-node-shell</strong> chưa được cài trên management cluster — nút SSH sẽ hiển thị hướng dẫn cài đặt.</span>
                <a href="https://github.com/kvaps/kubectl-node-shell" target="_blank" rel="noopener noreferrer"
                  className="underline ml-1 hover:text-amber-900 flex items-center gap-0.5">
                ...
                {machines.length === 0 ? (
                  ...
                  <tr key={m.name} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      {m.failureMessage && <p className="text-xs text-rose-400 mt-0.5 max-w-xs truncate">{m.failureMessage}</p>}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        onClick={() => setSshMachine(m)}
                        disabled={m.phase !== "Running"}
                        title={m.phase !== "Running" ? "Machine chưa Running" : `Shell vào ${m.nodeName || m.name}`}
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-500 font-medium border-b">
                <tr>
                  <th className="px-6 py-3">Type</th><th className="px-6 py-3">Status</th>
                  <th className="px-6 py-3">Reason</th><th className="px-6 py-3">Last Transition</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
